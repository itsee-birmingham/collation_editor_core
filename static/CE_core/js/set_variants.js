  ...
  return {
    ...
    showSetVariants: function(options) {
      ...
      if (Object.prototype.hasOwnProperty.call(options, 'container')) {
        if (CL.container !== options.container) {
          CL.container = options.container;
        }
      } else {
        CL.container = document.getElementsByTagName('body')[0];
      }
      ...
      SV.showSetVariantsData(options);
    ...
    showSetVariantsData: function(options) {
      ...
      for (let i = 0; i < _watchList.length; i += 1) {
        if (SV._checkWitnessIntegrity(_watchList[i])) {
          _watchList[i] = null;
        }
      }
      ...
      if (Object.prototype.hasOwnProperty.call(options, 'message') && _watchList.length > 0) {
        SV._setupMessage('warning', options.message.message + '<br/><br/>WARNING: the following witnesses still have ' +
                      'words out of order: ' + _watchList.join(', '));
      } else if (Object.prototype.hasOwnProperty.call(options, 'message')) {
        SV._setupMessage(options.message.type, options.message.message);
      } else if (_watchList.length > 0) {
        SV._setupMessage('warning', 'WARNING: the following witnesses still have words out of order: ' +
                      _watchList.join(', '));
      }
      CL.expandFillPageClients();
    },
    ...
    calculateUnitLengths: function(appId, options) {
      let index, id, start, firstHit, gapBefore, lastEnd, length, gapCounts, highestGap, gapAfter,
          previousUnitGapAfter, previousUnitEnd;
      if (options === undefined) {
        options = {};
      }
      const topLine = CL.data.apparatus;
      const app = CL.data[appId];
      ...
      for (let i = 0; i < app.length; i += 1) {
        ...
        for (let j = 0; j < topLine.length; j += 1) {
          if (Object.prototype.hasOwnProperty.call(topLine[j], 'overlap_units') &&
                  Object.prototype.hasOwnProperty.call(topLine[j].overlap_units, id) && start === -1) {
        ...
        for (let j = 0; j < topLine.length; j += 1) {
          if (topLine[j].start === start) {
            if (Object.prototype.hasOwnProperty.call(topLine[j], 'overlap_units') &&
                    Object.prototype.hasOwnProperty.call(topLine[j].overlap_units, id)) {
              if (firstHit === -1) {
                firstHit = j;
              }
            } else {
        ...
        while (index < topLine.length) {
          if (Object.prototype.hasOwnProperty.call(topLine[index], 'overlap_units') &&
                    Object.prototype.hasOwnProperty.call(topLine[index].overlap_units, id)) {
            ...
            if (topLine[index].start % 2 === 1) {  // if we start with odd number
              ...
              if (Object.prototype.hasOwnProperty.call(gapCounts, topLine[index].start)) {
        ...
        if (lastEnd % 2 === 1) {
          if (Object.prototype.hasOwnProperty.call(gapCounts, highestGap)) {
            ...
            if (Object.prototype.hasOwnProperty.call(options.column_lengths, highestGap) &&
    ...
    getUnitData: function(data, id, start, end, options) {
      ...
      const svRules = CL.getRuleClasses(undefined, undefined, 'value', ['identifier', 'keep_as_main_reading', 'suffixed_label', 'suffixed_reading']);
      for (const key in svRules) {
        if (Object.prototype.hasOwnProperty.call(svRules, key) && svRules[key][1] === false) {
          delete svRules[key];
        }
      }
      ...
      if (Object.prototype.hasOwnProperty.call(options, 'split') && options.split === true &&
              Object.prototype.hasOwnProperty.call(options, 'overlap_units')) {
        for (const key in options.overlap_units) {
      ...
      for (let i = 0; i < data.length; i += 1) {
        ...
        if (Object.prototype.hasOwnProperty.call(options, 'split') && options.split === true) {
          ...
          temp = SV._showSubreadings(data[i], id, i, highlightedHand, options.highlighted_added_wits);
          html.push.apply(html, temp[0]);
    ...
    reindexUnit: function(location) {
      ...
      for (let i = 0; i < CL.data.apparatus.length; i += 1) {
        if (CL.data.apparatus[i].start === location) {
          ...
          firstWordIndex = SV._reindexReadings(CL.data.apparatus[i].readings, location, firstWordIndex);
        }
      }
    },
    ...
    checkCombinedGapFlags: function(reading) {
      if (reading.text.length > 0) {
        if (Object.prototype.hasOwnProperty.call(reading.text[0], 'combined_gap_before')) {
        ...
        if (Object.prototype.hasOwnProperty.call(reading.text[0], 'combined_gap_after')) {
    ...
    doSplitReadingWitnesses: function(unitNum, readingPos, witnessList, apparatus, justSplit, idForNewReading) {
      ...
      const originalUnit = CL.data[apparatus][unitNum];
      const reading = originalUnit.readings[readingPos];
      if (justSplit) {
      ...
      const newReading = JSON.parse(JSON.stringify(reading));
      // remove readings on witnessList from the original reading
      for (let i = 0; i < witnessList.length; i += 1) {
        SV._removeWitnessFromReading(reading, witnessList[i]);
      }
      // if no witnesses left at all and there are no subreadings then delete the original reading
      if (reading.witnesses.length === 0 && !Object.prototype.hasOwnProperty.call(reading, 'subreadings')) {
        originalUnit.readings.splice(readingPos, 1);
        originalRemoved = true;
      } else {
        SV.checkCombinedGapFlags(reading);
        originalRemoved = false;
      }
      // remove all not on witnessList from newReading
      const newWitnessList = CL.getAllReadingWitnesses(newReading);  // a copy so we aren't manipulating what we are looping!
      for (let i = 0; i < newWitnessList.length; i += 1) {
        if (witnessList.indexOf(newWitnessList[i]) === -1) {
          SV._removeWitnessFromReading(newReading, newWitnessList[i]);
        }
      }
      SV.checkCombinedGapFlags(newReading);
      if (typeof idForNewReading !== 'undefined') {
        ...
        if (!Object.prototype.hasOwnProperty.call(newReading, '_id')) {
          ...
          if (index !== -1 && text.length > 0) {  // if we have text and the reading is already in the list of readings
            ...
            for (let j = 0; j < reading.witnesses.length; j += 1) {
              ...
              for (let k = 0; k < reading.text.length; k += 1) {
                if (Object.prototype.hasOwnProperty.call(reading.text[k], witness)) {
                ...
                if (Object.prototype.hasOwnProperty.call(unit.readings[index].text[k][witness], 'decision_class')) {
    ...
    splitReadingWitnesses: function(rdgDetails, stage, menuPos) {
      ...
      if (CL.getAllReadingWitnesses(reading).length > 1) {
        ...
        SV._setupMessage('error', 'ERROR: Readings with a single witness cannot be split');
      }
    },
    ...
    prepareForOperation: function() {
      SV._removeOffsetSubreadings();  // this runs find_subreadings
      SR.loseSubreadings();
    },
    ...
    makeStandoffReading: function(type, readingDetails, parentReading, outerCallback) {
      ...
      const callback = function() {
        SV.checkBugStatus('make offset reading', 'of type ' + type + ' in apparatus unit ' + readingDetails.unit_pos +
      ...
      for (const key in CL.data) {
        if (key.indexOf('apparatus') !== -1) {
          for (let i = 0; i < CL.data[key].length; i += 1) {
            if (!Object.prototype.hasOwnProperty.call(CL.data[key][i], '_id')) {
              return [true, ' there is no id on unit ' + i + ' in ' + key];
            }
            for (let j = 0; j < CL.data[key][i].readings.length; j += 1) {
              if (!Object.prototype.hasOwnProperty.call(CL.data[key][i].readings[j], '_id')) {
    ...
    checkBugStatus: function(action, details) {
      let bugReport, newWindow1, tmp;
      if (!Object.prototype.hasOwnProperty.call(CL.data, 'event_list')) {
        CL.data.event_list = [];
      }
      ...
      const indexProblem = SV._checkIndexesPresent();
      const witnessProblem = SV._checkUniqueWitnesses();
      const siglaProblem = SV._checkSiglaProblems();
      const standoffProblem = SV.checkStandoffReadingProblems();
      const tnCheck = SV._checkTAndNPresence();
      const idProblem = SV.checkIds();
            ...
            for (let i = 0; i < CL.data[key].length; i += 1) {
              ...
              for (let j = 0; j < unit.readings.length; j += 1) {
                ...
                if (Object.prototype.hasOwnProperty.call(reading, 'SR_text') ||
                  ...
                  if (Object.prototype.hasOwnProperty.call(reading, 'SR_text')) {
                    for (const witness in reading.SR_text) {
                      if (Object.prototype.hasOwnProperty.call(reading.SR_text, witness)) {
                        result = SV._checkForStandoffReading(witness, unit);
                        if (result === false) {
                  ...
                  if (Object.prototype.hasOwnProperty.call(reading, 'standoff_subreadings')) {
                    for (let k = 0; k < reading.standoff_subreadings.length; k += 1) {
                      result = SV._checkForStandoffReading(reading.standoff_subreadings[k], unit);
                      if (result === false) {
    ...
    areAllUnitsComplete: function() {
      ...
      for (let i = 0; i < CL.data.apparatus.length; i += 1) {
        witnesses = SV._getAllUnitWitnesses(CL.data.apparatus[i]);
    ...
    separateOverlapWitnesses: function(unitNum, gapLocation) {
      let overlappedWitnesses, toAdd, unit, toSplit, newReadingId, newReading;
      // get the overlapped witnesses for this point
      if (typeof gapLocation !== 'undefined') {
        overlappedWitnesses = SV._getOverlappedWitnessesForGap(gapLocation);
      } else {
        overlappedWitnesses = SV._getOverlappedWitnessesForUnit(unitNum);
      }
      if (overlappedWitnesses.length > 0) {
        unit = CL.data.apparatus[unitNum];
        for (let i = unit.readings.length - 1; i >= 0; i -= 1) {
          if (!Object.prototype.hasOwnProperty.call(unit.readings[i], 'overlap_status') ||
            (Object.prototype.hasOwnProperty.call(unit.readings[i], 'overlap_status') &&
            ...
            if (toSplit.length > 0) {
              toAdd = SV._separateIndividualOverlapWitnesses(toSplit, unit.overlap_units);
              for (let j = 0; j < toAdd.length; j += 1) {
                newReadingId = SV.doSplitReadingWitnesses(unitNum, i, toAdd[j][0], 'apparatus');
                newReading = CL.findReadingById(unit, newReadingId);
                if (Object.prototype.hasOwnProperty.call(toAdd[j][1], 'type')) {
                ...
                if (Object.prototype.hasOwnProperty.call(toAdd[j][1], 'details')) {
    ...
    combineReadings: function(readings1, readings2, newunit, moveReading) {
      ...
      for (let i = 0; i < readings1.length; i += 1) {
        ...
        for (let j = 0; j < readings1[i].witnesses.length; j += 1) {
          ...
          if (read2 !== null && Object.prototype.hasOwnProperty.call(readings2[read2], 'overlap_status')) {
          ...
          if (Object.prototype.hasOwnProperty.call(newReadings, text)) {
            SV._addTypeAndDetails(newReadings[text], readings1[read1], readings2[read2]);
            newReadings[text].witnesses.push(witness);
            newReadings[text].text = SV._combineReadingText(readings1, readings2,
                                                        [read1, read2], witness,
            ...
            if (read2 !== null) {  // we only need to worry about which one to chose if we have two readings
              if (Object.prototype.hasOwnProperty.call(readings1[read1], 'overlap_status') &&
                      Object.prototype.hasOwnProperty.call(readings2[read2], 'overlap_status')) {
              ...
              } else if (Object.prototype.hasOwnProperty.call(readings1[read1], 'overlap_status') ||
                            Object.prototype.hasOwnProperty.call(readings2[read2], 'overlap_status')) {
    ...
    * (only in CL.witnessAddingMode), called from select box in page footer */
    ...
    _reindexReadings: function(readings, location, firstWordIndex) {
      ...
      for (let i = 0; i < readings.length; i += 1) {
        ...
        if (readings[i].text.length === 0) {
          ...
          if (Object.prototype.hasOwnProperty.call(readings[i], 'index')) {
      ...
      for (let i = 0; i < CL.data.apparatus.length; i += 1) {
        if (CL.data.apparatus[i].start === location) {
          for (let j = 0; j < CL.data.apparatus[i].readings.length; j += 1) {
            SV._checkAndFixReadingIndexes(CL.data.apparatus[i].readings[j]);
          }
        }
      }
    },
    ...
    _checkAndFixReadingIndexes: function(reading) {
      for (let i = 0; i < reading.text.length; i += 1) {
        if (i + 1 < reading.text.length) {
          if (!SV._indexLessThan(reading.text[i].index, reading.text[i + 1].index)) {
            reading.text[i + 1].index = SV._incrementSubIndex(reading.text[i].index, 1);
          }
        }
      }
    },
    ...
    _unsplitReadings: function(details) {
      const scrollOffset = [document.getElementById('scroller').scrollLeft,
                            document.getElementById('scroller').scrollTop];
      SV.prepareForOperation(); //we do this because we have to sort out split witnesses as well
      delete CL.data[details[1]][details[0]].split_readings;
      SV.unsplitUnitWitnesses(details[0], details[1]);
      SV.unprepareForOperation();
      SV.checkBugStatus('recombine', 'readings in unit ' + details[0] + 'in  apparatus ' + details[1] + '.');
      SV.showSetVariantsData();
      document.getElementById('scroller').scrollLeft = scrollOffset[0];
      document.getElementById('scroller').scrollTop = scrollOffset[1];
    },
    ...
    _removeWitnessFromTokens: function(tokens, witness) {
      ...
      for (let i = 0; i < tokens.length; i += 1) {
        ...
        if (Object.prototype.hasOwnProperty.call(tokens[i], witness)) {
    ...
    */
    _removeWitnessFromReading: function(reading, witness) {
      if (reading.witnesses.indexOf(witness) !== -1) {
        ...
        SV._removeWitnessFromTokens(reading.text, witness);
      }
            ...
            for (let i = 0; i < reading.subreadings[key].length; i += 1) {
              if (reading.subreadings[key][i].witnesses.indexOf(witness) !== -1) {
                ...
                if (reading.subreadings[key][i] !== null) {
                  SV._removeWitnessFromTokens(reading.subreadings[key][i].text, witness);
                }
              }
            }
      ...
      if (Object.prototype.hasOwnProperty.call(reading, 'standoff_subreadings') && reading.standoff_subreadings.indexOf(witness) !== -1) {
      ...
      if (Object.prototype.hasOwnProperty.call(reading, 'combined_gap_before_subreadings') &&
        ...
        if (Object.prototype.hasOwnProperty.call(reading, 'combined_gap_before_subreadings_details')) {
    ...
    _getOverlappedWitnessesForUnit: function(unitPos) {
      const witnesses = [];
      const unit = CL.data.apparatus[unitPos];
    ...
    * point to odd numbered index point)*/
    _doMoveWholeUnit: function(targetLocation, originalLocation, unitNum) {
      ...
      if ((!Object.prototype.hasOwnProperty.call(CL.data.apparatus[unitNum], 'overlap_units') &&
          !SV._targetHasOverlapConflict(targetLocation, unitNum)) ||
        (Object.prototype.hasOwnProperty.call(CL.data.apparatus[unitNum], 'overlap_units') &&
          JSON.stringify(CL.data.apparatus[unitNum].overlap_units) === JSON.stringify(SV._getOverlapDetailsForGap(targetLocation)))) {
        SV.prepareForOperation();
        SV._addToUndoStack(CL.data);
        ...
        if (SV._getOverlappedWitnessesForGap(targetLocation).length > 0) {
          CL.data.apparatus[unitNum].overlap_units = SV._getOverlapDetailsForGap(targetLocation);
    ...
    _targetHasOverlapConflict: function(gapLocation, unitNum, reading) {
      let unit;
      const overlappedWitnesses = SV._getOverlappedWitnessesForGap(gapLocation);
      if (overlappedWitnesses.length > 0) {
    ...
    _sourceHasOverlapConflict: function(unitNum, reading) {
      const unit = CL.data.apparatus[unitNum];
      if (!Object.prototype.hasOwnProperty.call(unit, 'overlap_units')) {
        return false;
      }
      const overlappedWitnesses = [];
      for (const key in unit.overlap_units) {
        if (Object.prototype.hasOwnProperty.call(unit.overlap_units, key)) {
    ...
    _allOverlapsMatch: function(gapLocation, unitNum) {
      ...
      const gapOverlappedWitnesses = SV._getOverlapDetailsForGap(gapLocation);
      const unitOverlappedWitnesses = unit.overlap_units;
      for (const key in unitOverlappedWitnesses) {
        if (Object.prototype.hasOwnProperty.call(unitOverlappedWitnesses, key) && Object.prototype.hasOwnProperty.call(gapOverlappedWitnesses, key)) {
    ...
    _neighboursShareOverlaps: function(indexPoint, data) {
      ...
      if (!Object.prototype.hasOwnProperty.call(before, 'overlap_units') &&
    ...
    _doMoveSingleReading: function(targetLocation, originalLocation, unitNum, rd) {
      let witnesses, newunit, readings, problems, warningMess,
          replacementReadings, newUnitId, newUnitPos, readingPos;

      // move the single reading
      const scrollOffset = [document.getElementById('scroller').scrollLeft,
                            document.getElementById('scroller').scrollTop];
      // get some details we need to check for overlap
      const unit2 = CL.data.apparatus[unitNum];
      const rdgDetails = CL.getUnitAppReading(rd.obj.id);
      //get the reading you've moved and copy it otherwise prepareForOperation will lose data
      const reading = JSON.parse(JSON.stringify(unit2.readings[rdgDetails[2]]));
      if (!Object.prototype.hasOwnProperty.call(reading, 'overlap_status') || reading.overlap_status === 'duplicate') {
        if ((!SV._targetHasOverlapConflict(targetLocation, unitNum, reading) &&
              !SV._sourceHasOverlapConflict(unitNum, reading)) ||
              (SV._allOverlapsMatch(targetLocation, unitNum) && reading.text.length > 0)) {
          ...
          if (Object.prototype.hasOwnProperty.call(unit2, 'overlap_units') && SV._neighboursShareOverlaps(targetLocation)) {
          ...
          replacementReadings = SV._getReplacementOmReading(unit2, reading);
          ...
          SV.unsplitUnitWitnesses(unitNum, 'apparatus'); //merge with any other oms if needed
          ...
          if (SV._getOverlappedWitnessesForGap(targetLocation).length > 0) {
            CL.data.apparatus[newUnitPos].overlap_units = SV._getOverlapDetailsForGap(targetLocation);
            SV.separateOverlapWitnesses(newUnitPos, targetLocation);
          }
          // see if the unit that things were moved out of needs deleting or not
          if (!CL.unitHasText(unit2)) {
            if (!Object.prototype.hasOwnProperty.call(unit2, 'overlap_units')) {
          ...
          problems = SV._checkWordOrderIntegrity(originalLocation, targetLocation, reading.witnesses);
          if (problems.length > 0) {
    ...
    _getOverlapDetailsForGap: function(gapIndex) {
      ...
      for (let i = 0; i < CL.data.apparatus.length; i += 1) {
        ...
        if (typeof unitBefore !== 'undefined' && typeof unitAfter !== 'undefined') {
          if (Object.prototype.hasOwnProperty.call(unitBefore, 'overlap_units') &&
                    Object.prototype.hasOwnProperty.call(unitAfter, 'overlap_units')) {
            for (const key in unitBefore.overlap_units) {
              if (Object.prototype.hasOwnProperty.call(unitBefore.overlap_units, key) &&
                      Object.prototype.hasOwnProperty.call(unitAfter.overlap_units, key)) {
    ...
    _getOverlappedWitnessesForGap: function(gapIndex) {
      ...
      for (let i = 0; i < apparatus.length; i += 1) {
        ...
        if (typeof unitBefore !== 'undefined' && typeof unitAfter !== 'undefined') {
          if (Object.prototype.hasOwnProperty.call(unitBefore, 'overlap_units') && Object.prototype.hasOwnProperty.call(unitAfter, 'overlap_units')) {
            for (const key in unitBefore.overlap_units) {
              if (Object.prototype.hasOwnProperty.call(unitBefore.overlap_units, key) && Object.prototype.hasOwnProperty.call(unitAfter.overlap_units, key)) {
      ...
      if (targetLocation % 2 === 1) {
        if ((unit.start === unit.end && unit.start % 2 === 1) || typeof unitDetails[2] !== 'undefined') {
          if (unitDetails[1] === 'apparatus') {  // check this is a main reading not an overlapping one
            if (!SV._unitAtLocation(targetLocation)) {
              if (!Object.prototype.hasOwnProperty.call(CL.data.apparatus[unitNum], 'split_readings')) {
                SV._doMoveWholeUnit(targetLocation, originalLocation, unitNum);
              } else {
                SV._doMoveSingleReading(targetLocation, originalLocation, unitNum, rd);
              }
            } else {
    ...
    _checkWitnessEquality: function(unit1, unit2, appId) {
      const witnesses1 = CL.getActiveUnitWitnesses(unit1, appId);
      const witnesses2 = CL.getActiveUnitWitnesses(unit2, appId);
    ...
    _doCombineUnits: function(units, appId, keepId) {
      ...
      const unit1 = CL.data[appId][Math.min(units[0][0], units[1][0])];
      const unit2 = CL.data[appId][Math.max(units[0][0], units[1][0])];
      ...
      if (witnessEquality && overlapBoundaries && overlapStatusAgreement) {
        if (keepId === true) {
          ...
          SV._combineApparatusIds(unit1._id, unit2._id);
        }
        ...
        for (let i = 0; i < unit1.readings.length; i += 1) {
          if (Object.prototype.hasOwnProperty.call(unit1.readings[i], 'combined_gap_before_subreadings')) {
            combinedGapBeforeSubreadings.push.apply(combinedGapBeforeSubreadings,
                                                    unit1.readings[i].combined_gap_before_subreadings);
          }
          if (Object.prototype.hasOwnProperty.call(unit1.readings[i], 'combined_gap_before_subreadings_details')) {
            for (const key in unit1.readings[i].combined_gap_before_subreadings_details) {
              if (Object.prototype.hasOwnProperty.call(unit1.readings[i].combined_gap_before_subreadings_details, key)) {
        ...
        for (let i = 0; i < unit2.readings.length; i += 1) {
          if (Object.prototype.hasOwnProperty.call(unit2.readings[i], 'combined_gap_after_subreadings')) {
        ...
        newunit.first_word_index = SV._getLowestIndex(unit1.first_word_index, unit2.first_word_index);
        if (Object.prototype.hasOwnProperty.call(unit1, 'row')) {
          newunit.row = unit1.row;
        }
  
        if (Object.prototype.hasOwnProperty.call(unit1, 'overlap_units')) {  // at this point they are the same so we can just take one
        ...
        for (let i = 0; i < combinedGapBeforeSubreadings.length; i += 1) {
          for (let j = 0; j < newunit.readings.length; j += 1) {
            if (newunit.readings[j].witnesses.indexOf(combinedGapBeforeSubreadings[i]) !== -1) {
              if (Object.prototype.hasOwnProperty.call(newunit.readings[j], 'combined_gap_before_subreadings')) {
              ...
              if (Object.prototype.hasOwnProperty.call(combinedGapBeforeSubreadingsDetails, combinedGapBeforeSubreadings[i])) {
                if (Object.prototype.hasOwnProperty.call(newunit.readings[j], 'combined_gap_before_subreadings_details')) {
        ...
        for (let i = 0; i < combinedGapAfterSubreadings.length; i += 1) {
          for (let j = 0; j < newunit.readings.length; j += 1) {
            if (newunit.readings[j].witnesses.indexOf(combinedGapAfterSubreadings[i]) !== -1) {
              if (Object.prototype.hasOwnProperty.call(newunit.readings[j], 'combined_gap_after_subreadings')) {
        ...
        for (let i = 0; i < newunit.readings.length; i += 1) {
          SV.checkCombinedGapFlags(newunit.readings[i]);
        }
        // remove old units;
        ...
        SV.separateOverlapWitnesses(index);
        if (newunit.start === newunit.end) {
        ...
        if (appId !== 'apparatus') {
          ...
          for (let i = 0; i < CL.data[appId][index].readings.length; i += 1) {
            if (CL.data[appId][index].readings[i].witnesses.indexOf(CL.dataSettings.base_text_siglum) !== -1) {
              // split it out
              SV.doSplitReadingWitnesses(index, i, [CL.dataSettings.base_text_siglum], appId);
    ...
    _checkOverlapBoundaries: function(unit1, unit2, appId, allGaps) {
      let unit1Ids, unit2Ids;
      if (appId !== 'apparatus') {
        return true;
      }
      if ((!Object.prototype.hasOwnProperty.call(unit1, 'overlap_units') || $.isEmptyObject(unit1.overlap_units)) &&
            (!Object.prototype.hasOwnProperty.call(unit2, 'overlap_units') || $.isEmptyObject(unit2.overlap_units))) {
        return true;
      }
      if (allGaps === true) {
        if (!CL.unitHasText(unit1) && (!Object.prototype.hasOwnProperty.call(unit1, 'overlap_units') ||
                                        $.isEmptyObject(unit1.overlap_units))) {
          return true;
        }
        if (!CL.unitHasText(unit2) && (!Object.prototype.hasOwnProperty.call(unit2, 'overlap_units') ||
                                        $.isEmptyObject(unit2.overlap_units))) {
          return true;
        }
      }
      if (Object.prototype.hasOwnProperty.call(unit1, 'overlap_units') && Object.prototype.hasOwnProperty.call(unit2, 'overlap_units')) {
        unit1Ids = SV._getObjectKeys(unit1.overlap_units);
        unit2Ids = SV._getObjectKeys(unit2.overlap_units);
    ...
    _checkSpecialOverlapStatusAgreement: function(textUnit, emptyUnit) {
      const witnesses = [];
      for (const key in textUnit.overlap_units) {
        if (Object.prototype.hasOwnProperty.call(textUnit.overlap_units, key)) {
          witnesses.push.apply(witnesses, textUnit.overlap_units[key]);
        }
      }
      for (let i = 0; i < emptyUnit.readings.length; i += 1) {
        if (Object.prototype.hasOwnProperty.call(emptyUnit.readings[i], 'type') && emptyUnit.readings[i].type === 'om') {
    ...
    _checkOverlapStatusAgreement: function(unit1, unit2, appId, allGaps) {
      ...
      if (allGaps === true) {
        if (!CL.unitHasText(unit1) && !Object.prototype.hasOwnProperty.call(unit1, 'overlap_units') &&
                  Object.prototype.hasOwnProperty.call(unit2, 'overlap_units')) {
          return SV._checkSpecialOverlapStatusAgreement(unit2, unit1);
        }
        if (!CL.unitHasText(unit2) && !Object.prototype.hasOwnProperty.call(unit2, 'overlap_units') &&
                  Object.prototype.hasOwnProperty.call(unit1, 'overlap_units')) {
          return SV._checkSpecialOverlapStatusAgreement(unit1, unit2);
        }
      }
      const specialWits1 = {};
      for (let i = 0; i < unit1.readings.length; i += 1) {
        if (Object.prototype.hasOwnProperty.call(unit1.readings[i], 'overlap_status')) {
          if (!Object.prototype.hasOwnProperty.call(specialWits1, unit1.readings[i].overlap_status)) {
      ...
      const specialWits2 = {};
      for (let i = 0; i < unit2.readings.length; i += 1) {
        if (Object.prototype.hasOwnProperty.call(unit2.readings[i], 'overlap_status')) {
          if (!Object.prototype.hasOwnProperty.call(specialWits2, unit2.readings[i].overlap_status)) {
      ...
      const keys1 = SV._getObjectKeys(specialWits1);
      const keys2 = SV._getObjectKeys(specialWits2);
    ...
    /* TODO: is this used? doesn't seem to be in this file */
    ...
    _specialCombineReadings: function(emptyUnit, textUnit, textUnitPos, gapPosition, appId, adjacentUnit) {
      let hitWitnesses, newReadingId, reading, gapDetails;
      const targetWitnesses = [];
      const adjacentWitnesses = [];
      const readingsToAdd = [];
      ...
      for (let i = readingsToAdd.length - 1; i >= 0; i -= 1) {
        newReadingId = SV.doSplitReadingWitnesses(readingsToAdd[i][0], readingsToAdd[i][1],
        ...
        if (reading.text.length > 0) {
          if (gapPosition === 'before') {
            ...
            if (Object.prototype.hasOwnProperty.call(reading.text[0][readingsToAdd[i][2][0]], 'gap_before_details')) {
              reading.text[0].combined_gap_before_details = reading.text[0][readingsToAdd[i][2][0]].gap_before_details;
            }
            if (typeof adjacentUnit !== 'undefined') {
              gapDetails = SV._getGapDetails(adjacentUnit, readingsToAdd[i][2]);
              if (gapDetails !== null) {
      ...
      SV.unsplitUnitWitnesses(textUnitPos, 'apparatus');
    },
    ...
    _doSpecialCombineUnits: function(units, appId) {
      ...
      SV._addToUndoStack(CL.data);
      // make sure unit 1 is leftmost and unit 2 is rightmost
      const unit1Pos = Math.min(units[0][0], units[1][0]);
      const unit2Pos = Math.max(units[0][0], units[1][0]);
      const unit1 = CL.data[appId][unit1Pos];
      const unit2 = CL.data[appId][unit2Pos];
      const witnessEquality = SV._checkWitnessEquality(unit1, unit2, appId);
      if (witnessEquality) {
        if (!CL.unitHasText(unit1)) {
          // we need the unit before to get data from it
          if (unit1Pos !== 0) {
            SV._specialCombineReadings(unit1, unit2, unit2Pos, 'before', appId, CL.data[appId][unit1Pos - 1]);
          } else {
            SV._specialCombineReadings(unit1, unit2, unit2Pos, 'before', appId);
          }
        }
        if (!CL.unitHasText(unit2)) {
          // we need the unit after to get data from it
          if (unit2Pos + 1 < CL.data[appId].length) {
            SV._specialCombineReadings(unit2, unit1, unit1Pos, 'after', appId, CL.data[appId][unit2Pos + 1]);
          } else {
            SV._specialCombineReadings(unit2, unit1, unit1Pos, 'after', appId);
          }
        }
        SV.unprepareForOperation();
        SV.checkBugStatus('special combine', appId + ' unit ' + Math.min(units[0][0], units[1][0]) + ' with unit ' +
    ...
    _combineUnits: function(units) {
      ...
      if (appId === 'apparatus') {
        if (Math.max(units[0][0], units[1][0]) - Math.min(units[0][0], units[1][0]) === 1) {
          ...
          if ((Object.prototype.hasOwnProperty.call(CL.data[appId][units[0][0]], 'created') &&
                    !CL.unitHasText(CL.data[appId][units[0][0]])) ||
                        (Object.prototype.hasOwnProperty.call(CL.data[appId][units[1][0]], 'created') &&
                              !CL.unitHasText(CL.data[appId][units[1][0]]))) {
            SV._doSpecialCombineUnits(units, appId);
          } else {
            SV._doCombineUnits(units, appId);
          }
        } else {
        ...
        if (SV._areAdjacent(unit1._id, unit2._id)) {
          SV._doCombineUnits(units, appId, true);
        } else {
          // redraw without making any changes
          errorMess = 'ERROR: non-adjacent units cannot be combined';
          SV.unprepareForOperation();
    ...
    _combineApparatusIds: function(unit1Id, unit2Id) {
      const topLine = CL.data.apparatus;
      const unitPositions = SV._findApparatusPositionsByOverlapId(unit1Id, unit2Id);
      const witDetails = topLine[unitPositions[0]].overlap_units[unit1Id];
      for (let i = unitPositions[1]; i < topLine.length; i += 1) {
        if (Object.prototype.hasOwnProperty.call(topLine[i], 'overlap_units') &&
                Object.prototype.hasOwnProperty.call(topLine[i].overlap_units, unit2Id)) {
    ...
    _findApparatusPositionsByOverlapId: function(unit1Id, unit2Id) {
      ...
      for (let i = 0; i < topLine.length; i += 1) {
        if (Object.prototype.hasOwnProperty.call(topLine[i], 'overlap_units')) {
          if (Object.prototype.hasOwnProperty.call(topLine[i].overlap_units, unit1Id)) {
            last1 = i;
          }
          if (Object.prototype.hasOwnProperty.call(topLine[i].overlap_units, unit2Id) && typeof first2 === 'undefined') {
            first2 = i;
          }
        }
      }
    ...
    _moveReading: function(units) {
      let unit1, unit2ReadingPos, problems, warningMess, reading, doMoveOut, doMoveIn, replacementReadings;
      const scrollOffset = [document.getElementById('scroller').scrollLeft,
                            document.getElementById('scroller').scrollTop];
      // at this point both units are from same apparatus
      const appId = units[0][1];
      ...
      const unit2 = CL.data[appId][units[1][0]];
      ...
      const witnesses = reading[0].witnesses.join(',');
      if (!Object.prototype.hasOwnProperty.call(reading[0], 'overlap_status') || reading[0].overlap_status === 'duplicate') {
        doMoveOut = true;
        ...
        if (Object.prototype.hasOwnProperty.call(unit2, 'overlap_units')) {
          for (const key in unit2.overlap_units) {
            if (Object.prototype.hasOwnProperty.call(unit2.overlap_units, key) && doMoveOut === true) {
              for (let i = 0; i < reading[0].witnesses.length; i += 1) {
                if (unit2.overlap_units[key].indexOf(reading[0].witnesses[i]) !== -1) {
                  if (!Object.prototype.hasOwnProperty.call(unit1, 'overlap_units') ||
                        (Object.prototype.hasOwnProperty.call(unit1, 'overlap_units') &&
                          !Object.prototype.hasOwnProperty.call(unit1.overlap_units, key))) {
                    doMoveOut = false;
                    break;
                  }
                }
        ...
        doMoveIn = true;
        if (Object.prototype.hasOwnProperty.call(unit1, 'overlap_units')) {
          for (const key in unit1.overlap_units) {
            if (Object.prototype.hasOwnProperty.call(unit1.overlap_units, key) && doMoveIn === true) {
              for (let i = 0; i < reading[0].witnesses.length; i += 1) {
                if (unit1.overlap_units[key].indexOf(reading[0].witnesses[i]) !== -1) {
                  if (!Object.prototype.hasOwnProperty.call(unit2, 'overlap_units') ||
                    (Object.prototype.hasOwnProperty.call(unit2, 'overlap_units') &&
                      !Object.prototype.hasOwnProperty.call(unit2.overlap_units, key))) {
                    doMoveIn = false;
                    break;
                  }
                }
        ...
        if (doMoveOut === true && doMoveIn === true) {
          ...
          // replace it with an om reading or two depending on overlap status
          replacementReadings = SV._getReplacementOmReading(unit2, reading[0]);
          for (let i = 0; i < replacementReadings.length; i += 1) {
            CL.addReadingId(replacementReadings[i], unit2.start, unit2.end);
            unit2.readings.push(replacementReadings[i]);
          }
          ...
          SV.unprepareForOperation();
          SV.checkBugStatus('move reading', unit2ReadingPos + ' with witnesses ' + witnesses + ' from unit ' +
    ...
    /* TODO: are the necxt two functions needed. Don't seem to be used in this file (second only used in first) */
    ...
    _neighboursShareAllOverlaps: function(appId, unitPos) {
      ...
      olIds = SV._getOverlappedIds(CL.data[appId][unitPos]);
      if (unitPos + 1 < CL.data[appId].length) {
        nextOlIds = SV._getOverlappedIds(CL.data[appId][unitPos + 1]);
      } else {
        nextOlIds = [];
      }
      if (unitPos !== 0) {
        previousOlIds = SV._getOverlappedIds(CL.data[appId][unitPos - 1]);
      } else {
        previousOlIds = [];
      }
    ...
    _getOverlappedIds: function(topUnit) {
      const ids = [];
      if (Object.prototype.hasOwnProperty.call(topUnit, 'overlap_units')) {
        for (const key in topUnit.overlap_units) {
          if (Object.prototype.hasOwnProperty.call(topUnit.overlap_units, key)) {
            ids.push(key);
          }
        }
      }
    ...
    _getReplacementOmReading: function(unit, movedReading) {
      if (Object.prototype.hasOwnProperty.call(movedReading, 'overlap_status')) {
      ...
      const witnesses = movedReading.witnesses.slice(0);
      ...
      for (let i = 1; i < reading.text.length; i += 1) {
        if (typeof reading.text[i].index !== 'undefined') {
          if (reading.text[i - 1].index === reading.text[i].index) {
            reading.text[i].index = SV._incrementSubIndex(reading.text[i].index, 1);
          }
        }
      }
      return reading;
    ...
    _orderUnitText: function(readings1, readings2, pattern, witness, changeIndexes) {
      ...
      words.sort(function(a, b) {
        if (Object.prototype.hasOwnProperty.call(a, witness) && Object.prototype.hasOwnProperty.call(b, witness)) {
      ...
      if (changeIndexes === true) {
        ...
        for (let i = 0; i < words.length; i += 1) {
          if (SV._indexLessThan(words[i].index, indexes[indexes.length - 1])) {
            ordered = false;
          }
          indexes.push(words[i].index);
        }
        if (ordered === false) {
          indexes.sort(SV._compareIndexStrings);
          for (let i = 0; i < words.length; i += 1) {
    ...
    _combineReadingText: function(readings1, readings2, pattern, witness, currentText) {
      ...
      const words = SV._orderUnitText(readings1, readings2, pattern, witness, true);
      k = 0;
      for (let i = 0; i < words.length; i += 1) {
        current = currentText[k] || {};
        currentText[k] = SV._combineWords(current, words[i], witness);
        if (k > 0 && Object.prototype.hasOwnProperty.call(currentText[k], 'combined_gap_before')) {
    ...
    _addTypeAndDetails: function(newreading, reading1, reading2) {
      ...
      if (typeof reading2 === 'undefined') {
        if (reading1.text.length === 0) {
          ...
          if (Object.prototype.hasOwnProperty.call(reading1, 'details')) {
        ...
        if (type1 === type2) {
          ...
          if (Object.prototype.hasOwnProperty.call(reading2, 'details')) {
      ...
      if (Object.prototype.hasOwnProperty.call(reading1, 'overlap')) {
    ...
    _combineWords: function(newWord, oldWord, witness) {
      if (!Object.prototype.hasOwnProperty.call(newWord, 'index')) {
        newWord.index = oldWord.index;
      }
      if (!Object.prototype.hasOwnProperty.call(newWord, 'verse')) {
        newWord.verse = oldWord.verse;
      }
      if (!Object.prototype.hasOwnProperty.call(newWord, 'interface')) {
        newWord['interface'] = oldWord['interface'];
      }
      if (!Object.prototype.hasOwnProperty.call(newWord, 'reading')) {
        newWord.reading = [];
      }
      if (Object.prototype.hasOwnProperty.call(oldWord, 'regularised')) {
        newWord.regularised = oldWord.regularised;
      }
      if (Object.prototype.hasOwnProperty.call(oldWord, 'was_gap')) {
        newWord.was_gap = oldWord.was_gap;
      }
      if (Object.prototype.hasOwnProperty.call(oldWord, 'combined_gap_before')) {
        if (!Object.prototype.hasOwnProperty.call(newWord, 'combined_gap_before')) {
      ...
      if (Object.prototype.hasOwnProperty.call(oldWord, 'combined_gap_before_details')) {
        newWord.combined_gap_before_details = oldWord.combined_gap_before_details;
      }
      if (Object.prototype.hasOwnProperty.call(oldWord, 'combined_gap_after')) {
        if (!Object.prototype.hasOwnProperty.call(newWord, 'combined_gap_after')) {
    ...
    _separateIndividualOverlapWitnesses: function(witnessList, overlaps) {
      let olAdds, olUnit, olRdgDetails;
      const newAdds = [];
      for (const key in overlaps) {
        if (Object.prototype.hasOwnProperty.call(overlaps, key)) {
          ...
          if (olUnit.readings.length > 1 && Object.prototype.hasOwnProperty.call(olUnit.readings[1], 'details')) {
            olRdgDetails.details = olUnit.readings[1].details;
          } else if (Object.prototype.hasOwnProperty.call(olUnit.readings[0], 'details')) {
    ...
    _doSplitUnit: function(unit, appId, index) {
      let witnesses, witnessesCopy, rdg, add, splitAdds, newunit, omReadingsCopy, lacReadingsCopy, newReading,
      ...
      const apps = [];
      witnesses = [];
      const overlapWitnesses = {};  // this is a keyed by overlap_status
      ...
      for (let i = 0; i < unit.readings.length; i += 1) {
        ...
        if (Object.prototype.hasOwnProperty.call(unit.readings[i], 'overlap_status')) {
          if (!Object.prototype.hasOwnProperty.call(overlapWitnesses, unit.readings[i].overlap_status)) {
        ...
        if (Object.prototype.hasOwnProperty.call(unit.readings[i], 'combined_gap_before_subreadings')) {
          delete unit.readings[i].combined_gap_before_subreadings;
          if (Object.prototype.hasOwnProperty.call(unit.readings[i], 'combined_gap_before_subreadings_details')) {
            delete unit.readings[i].combined_gap_before_subreadings_details;
          }
        }
        if (Object.prototype.hasOwnProperty.call(unit.readings[i], 'combined_gap_after_subreadings')) {
          delete unit.readings[i].combined_gap_after_subreadings;
        }
        if (unit.readings[i].text.length > 0) {
          if (Object.prototype.hasOwnProperty.call(unit.readings[i].text[0], 'combined_gap_before')) {
            delete unit.readings[i].text[0].combined_gap_before;
            if (Object.prototype.hasOwnProperty.call(unit.readings[i].text[0], 'combined_gap_before_details')) {
              delete unit.readings[i].text[0].combined_gap_before_details;
            }
          }
          if (Object.prototype.hasOwnProperty.call(unit.readings[i].text[unit.readings[i].text.length - 1], 'combined_gap_after')) {
      ...
      if (Object.prototype.hasOwnProperty.call(CL.data, 'lac_readings')) {
        for (let i = 0; i < CL.data.lac_readings.length; i += 1) {
          witnesses.splice(witnesses.indexOf(CL.data.lac_readings[i]), 1);
        }
      }
      ...
      words.sort(SV._compareIndexes);
      //build a dictionary of words using the word's index numbers as keys
      const wordsDict = {};
      for (let i = 0; i < words.length; i += 1) {
        if (Object.prototype.hasOwnProperty.call(wordsDict, words[i].index)) {
      ...
      for (const key in wordsDict) {
        if (Object.prototype.hasOwnProperty.call(wordsDict, key)) {
      ...
      for (let i = 0; i < text.length; i += 1) {
        ...
        for (let j = 0; j < text[i].length; j += 1) {
          if (typeof text[i][j].witnesses !== 'undefined') { //TODO: check if this condition is ever used
            ...
            if (Object.prototype.hasOwnProperty.call(text[i][j], 'overlap_status')) {
        ...
        if (witnessesCopy.length > 0) {
          for (const key in overlapWitnesses) {
            if (Object.prototype.hasOwnProperty.call(overlapWitnesses, key)) {
              add = [];
                  ...
                  if (Object.prototype.hasOwnProperty.call(unit, 'overlap_units')) {
                    ...
                    for (let k = 0; k < splitAdds.length; k += 1) {
                      if (key === 'duplicate') {
                        ...
                        if (Object.prototype.hasOwnProperty.call(splitAdds[k][1], 'type')) {
                          newReading.type = splitAdds[k][1].type;
                        } else {
                          newReading.type = 'om';
                        }
                        if (Object.prototype.hasOwnProperty.call(splitAdds[k][1], 'details')) {
        ...
        if (Object.prototype.hasOwnProperty.call(CL.data, 'om_readings') && CL.data.om_readings.length > 0) {
        ...
        if (Object.prototype.hasOwnProperty.call(CL.data, 'lac_readings') && CL.data.lac_readings.length > 0) {
      ...
      for (let i = apps.length - 1; i >= 0; i -= 1) {
        ...
        SV.unsplitUnitWitnesses(index, appId);
        ...
        if (appId !== 'apparatus') {
          for (let j = 0; j < unit.readings.length; j += 1) {
            if (unit.readings[j].witnesses.indexOf(CL.dataSettings.base_text_siglum) !== -1) {
              SV.doSplitReadingWitnesses(index, 0, [CL.dataSettings.base_text_siglum], appId);
              break;
            }
          }
        }
      ...
      SV.unprepareForOperation();
      SV.showSetVariantsData();
      SV.checkBugStatus('split', appId + ' unit ' + index + '.');
      document.getElementById('scroller').scrollLeft = scrollOffset[0];
      document.getElementById('scroller').scrollTop = scrollOffset[1];
    },
    ...
    _splitUnit: function(index) {
      let appId, apparatusNum, operationNeeded;
      const scrollOffset = [document.getElementById('scroller').scrollLeft,
                            document.getElementById('scroller').scrollTop];
      ...
      for (let i = 0; i < CL.data[appId][index].readings.length; i += 1) {
        if (CL.data[appId][index].readings[i].text.length > 1) {
          ...
          (Object.prototype.hasOwnProperty.call(CL.data[appId][index].readings[i].text[0], 'combined_gap_before') ||
                Object.prototype.hasOwnProperty.call(CL.data[appId][index].readings[i].text[0], 'combined_gap_after'))) {
          operationNeeded = true;
        } else if (Object.prototype.hasOwnProperty.call(CL.data[appId][index].readings[i], 'combined_gap_before_subreadings') ||
                        Object.prototype.hasOwnProperty.call(CL.data[appId][index].readings[i], 'combined_gap_after_subreadings')) {
          operationNeeded = true;
        }
      }
      if (operationNeeded === true) {
        SV._doSplitUnit(CL.data[appId][index], appId, index);
      } else {
        SV.showSetVariantsData();
        document.getElementById('scroller').scrollLeft = scrollOffset[0];
        document.getElementById('scroller').scrollTop = scrollOffset[1];
    ...
    _checkUnitUniqueness: function(unitNum, appId) {
      let prevStart, prevEnd, nextStart, nextEnd;
      // get start and end words of unit
      const start = CL.data[appId][unitNum].start;
      const end = CL.data[appId][unitNum].end;
    ...
    _getPosInUnitSet: function(unitNum, appId) {
      let i, prevStart, prevEnd, nextStart, nextEnd;
      const start = CL.data[appId][unitNum].start;
      const end = CL.data[appId][unitNum].end;
      const unitList = ['hit'];
      i = 1;
    ...
    _overlapReading: function(unitNum, readingNum, menuPos) {
      let reading, errorMess, duplicate, witnessList, data, newReadingId;
      const scrollOffset = [document.getElementById('scroller').scrollLeft,
                            document.getElementById('scroller').scrollTop];
      duplicate = true;
      if (SV._checkUnitUniqueness(unitNum, 'apparatus')) {
        reading = CL.data.apparatus[unitNum].readings[readingNum];
        if (!SV._hasStandoffSubreading(reading)) {
    ...
    _makeOverlappingReading: function(unitNum, readingId, duplicate) {
      let reading, found, newWord, temp;
      const scrollOffset = [document.getElementById('scroller').scrollLeft,
                            document.getElementById('scroller').scrollTop];
      SV._addToUndoStack(CL.data);
      const originalUnit = CL.data.apparatus[unitNum];
      const aReading = CL.data.apparatus[unitNum].readings[0];
      ...
      for (let i = 0; i < newunit.readings[0].text.length; i += 1) {
        for (const key in newunit.readings[0].text[i]) {
          if (Object.prototype.hasOwnProperty.call(newunit.readings[0].text[i], key)) {
      ...
      for (let i = reading.text.length - 1; i >= 0; i -= 1) {
        if (Object.prototype.hasOwnProperty.call(reading.text[i][reading.witnesses[0]], 'gap_after') &&
              (i + 1 >= reading.text.length || (i + 1 < reading.text.length &&
                !Object.prototype.hasOwnProperty.call(reading.text[i + 1], 'was_gap')))) {
          ...
          if (i < reading.text.length - 1 ||
                i === reading.text.length - 1 &&
                    Object.prototype.hasOwnProperty.call(newunit.readings[1].text[newunit.readings[1].text.length - 1], 'combined_gap_after')) {
            newWord = {
              'index': SV._incrementSubIndex(reading.text[i].index, 1),
            ...
            for (let j = 0; j < reading.witnesses.length; j += 1) {
              ...
              newWord[reading.witnesses[j]] = {
                'index': SV._incrementMainIndex(reading.text[i][reading.witnesses[j]].index, 1),
                'original': newWord['interface']
              };
              if (Object.prototype.hasOwnProperty.call(reading.text[i][reading.witnesses[j]], 'gap_after')) {
        ...
        if (i === 0 && Object.prototype.hasOwnProperty.call(newunit.readings[1].text[0], 'combined_gap_before')) {
          newWord = {
          ...
          if (parseInt(reading.text[0].index) % 2 === 0) { // check to see if it is an even numbered index
            temp = SV._decrementMainIndex(reading.text[0].index, 1);
            newWord.index = temp.split('.')[0] + '.99';
          } else {
            newWord.index = SV._decrementSubIndex(reading.text[i].index, 1);
            reading.text[i].index = SV._incrementSubIndex(newWord.index, 1);
          }
          for (let j = 0; j < reading.witnesses.length; j += 1) {
            newWord.reading.push(reading.witnesses[j]);
            newWord[reading.witnesses[j]] = {
            ...
            if (Object.prototype.hasOwnProperty.call(reading.text[i][reading.witnesses[j]], 'gap_before')) {
      ...
      SV.unprepareForOperation();
      SV.checkBugStatus('make overlapping reading', 'of witnesses ' +
    ...
    _checkWitnessIntegrity: function(witness) {
      ...
      for (let i = 0; i < CL.data.apparatus.length; i += 1) {  // step through the top line apparatus
        ...
        if (Object.prototype.hasOwnProperty.call(unit, 'overlap_units') && !Object.prototype.hasOwnProperty.call(unit.overlap_units, id)) {
          for (const key in unit.overlap_units) {
            if (Object.prototype.hasOwnProperty.call(unit.overlap_units, key) && unit.overlap_units[key].indexOf(witness) !== -1) {
              ...
              for (let j = 0; j < olUnit.readings.length; j += 1) {
                if (olUnit.readings[j].witnesses.indexOf(witness) !== -1) {
                  if (Object.prototype.hasOwnProperty.call(olUnit.readings[j], 'standoff_subreadings') &&
                        olUnit.readings[j].standoff_subreadings.indexOf(witness) !== -1) {
                    for (const type in olUnit.readings[j].subreadings) {
                  ...
                  } else if (Object.prototype.hasOwnProperty.call(olUnit.readings[j], 'SR_text') &&
                                Object.prototype.hasOwnProperty.call(olUnit.readings[j].SR_text, witness)) {
        ...
        if (!Object.prototype.hasOwnProperty.call(unit, 'overlap_units') || (Object.prototype.hasOwnProperty.call(unit, 'overlap_units') &&
              !Object.prototype.hasOwnProperty.call(unit.overlap_units, id))) {
          for (let j = 0; j < unit.readings.length; j += 1) {
            if (unit.readings[j].witnesses.indexOf(witness) !== -1) {
              //check the main reading
              if (unit.readings[j].text.length > 0 && Object.prototype.hasOwnProperty.call(unit.readings[j].text[0], witness)) {
              ...
              // check the SR_text
              if (Object.prototype.hasOwnProperty.call(unit.readings[j], 'SR_text') &&
                      Object.prototype.hasOwnProperty.call(unit.readings[j].SR_text, witness)) {
      ...
      for (let i = 0; i < unit.readings.length; i += 1) {
        for (let j = 0; j < unit.readings[i].witnesses.length; j += 1) {
          ...
          if (witnesses === undefined || witnesses.indexOf(wit) !== -1) {
            if (unit.readings[i].text.length > 0) {
              if (Object.prototype.hasOwnProperty.call(unit.readings[i].text[0], wit)) {
                ...
                if (Object.prototype.hasOwnProperty.call(unit.readings[i], 'SR_text') &&
                        Object.prototype.hasOwnProperty.call(unit.readings[i].SR_text, wit)) {
    ...
    _compareWitnessQueue: function(queue, problems) {
      ...
      for (const key in queue[0]) {
        if (problems.indexOf(key) === -1) {  // if we already know its a problem we can ignore it
          if (Object.prototype.hasOwnProperty.call(queue[0], key)) {
            if (Object.prototype.hasOwnProperty.call(queue[1], key)) {
    ...
    _checkWordOrderIntegrity: function(originalUnit, newUnit, witnesses) {
      let working, problems, unit;
      problems = [];
      working = false;
      const witnessQueue = [];
      const start = Math.min(originalUnit, newUnit);
      const end = Math.max(originalUnit, newUnit);
      const indexes = [];
      for (let i = 0; i < CL.data.apparatus.length; i += 1) {
        unit = CL.data.apparatus[i];
        ...
        if (unit.start >= start && working !== true) {
          working = true;
          witnessQueue.push(SV._getFirstIndexForWitnesses(unit, witnesses));
        } else if (unit.end > end) {
          if (witnessQueue.length === 2) {
            witnessQueue.shift();  // remove first item from list
          }
          witnessQueue.push(SV._getFirstIndexForWitnesses(unit, witnesses));
          problems = SV._compareWitnessQueue(witnessQueue, problems);
          break;
      ...
      for (let i = 0; i < indexes.length; i += 1) {
        ...
        problems.push.apply(problems, SV._checkUnitIntegrity('apparatus', indexes[i]));
      }
      problems = CL.setList(problems);
      ...
      for (let i = 0; i < indexes.length; i += 1) {
        if (Object.prototype.hasOwnProperty.call(CL.data.apparatus[indexes[i]], 'overlap_units')) {
          for (const key in CL.data.apparatus[indexes[i]].overlap_units) {
            if (Object.prototype.hasOwnProperty.call(CL.data.apparatus[indexes[i]].overlap_units, key)) {
    ...
    _checkUnitIntegrity: function(appId, index) {
      let problems, count, witness;
      problems = [];
      const unit = CL.data[appId][index];
      for (let i = 0; i < unit.readings.length; i += 1) {
        if (unit.readings[i].text.length > 0) {
          for (let j = 0; j < unit.readings[i].witnesses.length; j += 1) {
            ...
            for (let k = 0; k < unit.readings[i].text.length; k += 1) {
              if (Object.prototype.hasOwnProperty.call(unit.readings[i].text[k], witness)) {
    ...
    _removeOffsetSubreadings: function() {
      let apparatus, readings, parent, appString, subreadingId, rowNum, makeMainIds, makeMainIdsList;
        ...
        if (Object.prototype.hasOwnProperty.call(data, key)) {
          if (key.indexOf('apparatus') !== -1) {
            ...
            for (let i = 0; i < apparatus.length; i += 1) {
              ...
              for (const type in data.marked_readings) {
                if (Object.prototype.hasOwnProperty.call(data.marked_readings, type)) {
                  for (let k = 0; k < data.marked_readings[type].length; k += 1) {
                    if (data.marked_readings[type][k].apparatus === key) {  // if in right apparatus row
                      if (data.marked_readings[type][k].start === apparatus[i].start &&
                        ...
                        if (!Object.prototype.hasOwnProperty.call(data.marked_readings[type][k], 'first_word_index') ||
                          ...
                          for (let j = 0; j < readings.length; j += 1) {
                            if (data.marked_readings[type][k].parent_text !== '') {
                              ...
                              if (readings[j].text.length === 0) {
                                if (!Object.prototype.hasOwnProperty.call(data.marked_readings[type][k], 'om_details') &&
              ...
              if (!$.isEmptyObject(makeMainIds)) {
                makeMainIdsList = [];
                for (const idKey in makeMainIds) {
                  makeMainIdsList.push(idKey);
                }
                SV._makeMainReading(makeMainIdsList, makeMainIds);
              }
            }
          }
        }
    ...
    _makeMainReading: function(idString, details) {
      let scrollOffset, unitNumber, appId, unit, parentPos, subtype, subreadingPos, parentReading, options;
      if (Array.isArray(idString)) {
        ...
        idString.sort(SV._subreadingIdSort);
        ...
        SV.unprepareForOperation();
        SV.checkBugStatus('make main reading', idString);
        SV.showSetVariantsData();
        document.getElementById('scroller').scrollLeft = scrollOffset[0];
        document.getElementById('scroller').scrollTop = scrollOffset[1];
      }
    ...
    * options at the moment are 'deleted' and 'overlapped'*/
    ...
    _removeReadingFlag: function(readingDetails, reading) {
      ...
      SV.unsplitUnitWitnesses(readingDetails[0], readingDetails[1]);
      SV.showSetVariantsData();
      document.getElementById('scroller').scrollLeft = scrollOffset[0];
      document.getElementById('scroller').scrollTop = scrollOffset[1];
    },
    ...
    _addContextMenuHandlers: function() {
      ...
      if (document.getElementById('overlap')) {
        ...
        $('#overlap').on('click.or_c', function() {
          const element = SimpleContextMenu._target_element;
          const div = CL.getSpecifiedAncestor(element, 'DIV', function(e) {
            if ($(e).hasClass('spanlike')) {
              return false;
      ...
      if (document.getElementById('split-witnesses')) {
        ...
        $('#split-witnesses').on('click.sw_c', function() {
          ...
          const readingDetails = CL.getUnitAppReading(div.id);
          SV.splitReadingWitnesses(readingDetails, 'set_variants', {
            'top': SimpleContextMenu._menuElement.style.top,
            'left': SimpleContextMenu._menuElement.style.left
          });
        });
      ...
      for (let i = 0; i < CL.overlappedOptions.length; i += 1) {
        if (document.getElementById(CL.overlappedOptions[i].id)) {
          SV._addOverlappedEvent(CL.overlappedOptions[i].id, CL.overlappedOptions[i].reading_flag);
        }
      }
      // special added for SV
      const svRules = CL.getRuleClasses('create_in_SV', true, 'name',
    ...
    _addOverlappedEvent: function(id, flag) {
      ...
      $('#' + id).on('click.' + id + '_c', function() {
        ...
        const readingDetails = CL.getUnitAppReading(div.id);
        SV._addReadingFlag(readingDetails, flag);
      });
      $('#' + id).on('mouseover.' + id + '_mo', function() {
        CL.hideTooltip();
      });
    },
    ...
    _undo: function() {
      ...
      if (SV.undoStack.length > 0) {
        ...
        SV.checkBugStatus('undo', 'last action recorded');
        SV.showSetVariantsData();
        document.getElementById('scroller').scrollLeft = scrollOffset[0];
        document.getElementById('scroller').scrollTop = scrollOffset[1];
      }
    ...
    _checkTAndNPresence: function() {
      for (let i = 0; i < CL.data.apparatus.length; i += 1) {
        for (let j = 0; j < CL.data.apparatus[i].readings.length; j += 1) {
          if (Object.prototype.hasOwnProperty.call(CL.data.apparatus[i].readings[j], 'subreadings')) {
            for (const key in CL.data.apparatus[i].readings[j].subreadings) {
              for (let k = 0; k < CL.data.apparatus[i].readings[j].subreadings[key].length; k += 1) {
                for (let l = 0; l < CL.data.apparatus[i].readings[j].subreadings[key][l]; l += 1) {
                  for (let m = 0; m < CL.data.apparatus[i].readings[j].subreadings[key][l].text.length; m += 1) {
                    if (Object.prototype.hasOwnProperty.call(CL.data.apparatus[i].readings[j].subreadings[key][l].text[m], 't')) {
                      return [true, 'there is a rogue t in word ' + m + ' in subreading ' + l + ' of the ' + key +
                              ' subreadings of reading ' + j + ' of apparatus unit ' + i
                      ];
                    }
                    if (Object.prototype.hasOwnProperty.call(CL.data.apparatus[i].readings[j].subreadings[key][l].text[m], 'n')) {
          ...
          for (let k = 0; k < CL.data.apparatus[i].readings[j].text.length; k += 1) {
            if (Object.prototype.hasOwnProperty.call(CL.data.apparatus[i].readings[j].text[k], 't')) {
              return [true, 'there is a rogue t in word ' + k + ' in reading ' + j + ' of apparatus unit ' + i];
            }
            if (Object.prototype.hasOwnProperty.call(CL.data.apparatus[i].readings[j].text[k], 'n')) {
          ...
          if (key.match(/apparatus\d*/g) !== null) {
            for (let i = 0; i < CL.data[key].length; i += 1) {
              ...
              for (let j = 0; j < unit.readings.length; j += 1) {
                reading = unit.readings[j];
                for (let k = 0; k < reading.text.length; k += 1) {
                  if (Object.prototype.hasOwnProperty.call(reading.text[k], 'siglum')) {
          ...
          if (key.match(/apparatus\d*/g) !== null) {
            for (let i = 0; i < CL.data[key].length; i += 1) {
              ...
              for (let j = 0; j < unit.readings.length; j += 1) {
                ...
                for (let k = 0; k < reading.text.length; k += 1) {
                  word = reading.text[k];
                  if (!Object.prototype.hasOwnProperty.call(word, 'index') || typeof word.index === 'undefined') {
    ...
    //NB: we don't actually use this now but it might form the basis of being able to recombined overlapped readings with the top row
    /** merge two overlapping readings from the same column or merge an overlapped back into the main reading*/
    _mergeOverlaps: function(units) {
      let textString;
      const scrollOffset = [document.getElementById('scroller').scrollLeft,
                            document.getElementById('scroller').scrollTop];
      const unit1 = CL.data[units[0][1]][units[0][0]];
      const unit2 = CL.data[units[1][1]][units[1][0]];
      SV._addToUndoStack(CL.data);
      ...
      if (units[0][1] === 'apparatus') {
        for (let i = 1; i < unit2.readings.length; i += 1) {
          textString = CL.extractWitnessText(unit2.readings[i]);
          for (let j = 0; j < unit1.readings.length; j += 1) {
            if (CL.extractWitnessText(unit1.readings[j]) === textString) {
              for (let k = 0; k < unit1.readings[j].witnesses.length; k += 1) {
        ...
        if (unit2.readings.length === 1) {
          ...
          const warningMess = 'WARNING: Not all readings could be merged';
          SV.unsplitUnitWitnesses(units[0][0], units[0][1]);
          SV.checkBugStatus('merge overlap', 'merge unit ' + units[1][0] + ' from ' + units[1][1] + ' into unit ' + units[0][0] + ' in ' + units[0][1] + '.');
          SV.showSetVariantsData({
            'message': {
              'type': 'warning',
              'message': warningMess
      ...
      SV.unsplitUnitWitnesses(units[0][0], units[0][1]);
      SV.checkBugStatus('merge overlap', 'merge unit ' + units[1][0] + ' from ' + units[1][1] + ' into unit ' +
                        units[0][0] + ' in ' + units[0][1] + '.');
      SV.showSetVariantsData();
      document.getElementById('scroller').scrollLeft = scrollOffset[0];
      document.getElementById('scroller').scrollTop = scrollOffset[1];
    }
