...
class PostProcessor(Regulariser, SettingsApplier):
    ...
    def split_unit_into_single_words(self, readings_list, matrix, highest):
        """Split unit into single words (columns of matrix) and use vertically_combine_readings
        to combine any resulting shared units """
    ...
    def check_unit_splits(self, readings):
        ...
        if len(readings.keys()) > 1 and 'None' not in readings.keys() or \
           len(readings.keys()) > 2 and 'None' in readings.keys():
            ...
            # so just return existing readings except when we are combining new witnesses
            # into existing collations we always want the new reading in smallest chunks possible
            if len(readings.keys()) == 1 and self.split_single_reading_units is True:
                for key in readings:
    ...
    def anchor_readings(self, variant_units):
        ...
        for i, unit in enumerate(variant_units):
            ...
            if not len(base_reading) or (
                ...
                # used to report errors and use the logic from if above instead - ultimately data needs fixing
                # in these cases
                try:
